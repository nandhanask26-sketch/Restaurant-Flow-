...
export const InstallAppModal: React.FC<InstallAppModalProps> = ({
  ...
  const githubApkUrl = 'https://github.com/nandhanask26-sketch/Restaurant-Flow-/raw/main/RestaurantFlow.apk';
  const currentOrigin = typeof window !== 'undefined' ? window.location.origin : '';
  const localApkUrl = `${currentOrigin}/RestaurantFlow.apk`;

  // Branded display URL containing restaurantflow
  const brandedDisplayUrl = 'https://restaurantflow.app/download';
  ...
  const handleCopyApkLink = async () => {
    try {
      await navigator.clipboard.writeText(githubApkUrl);
      setCopied(true);
            ...
            <>
              ...
              <div className="space-y-2">
                <a
                  ...
                  className="btn-primary w-full py-4 text-sm font-black flex items-center justify-center gap-2.5 shadow-glow"
                >
                  <Download className="w-5 h-5" />
                  <span>Download RestaurantFlow.apk (Direct)</span>
                </a>
                <p className="text-[11px] text-center text-slate-400">
                  Direct installable APK package • Works on all Android smartphones
                </p>
              </div>
                ...
                <div className="space-y-2 text-center sm:text-left flex-1 min-w-0">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mt-0.5">
                      Point your phone camera at this QR code to download <strong className="text-brand-400">RestaurantFlow.apk</strong> immediately.
                    </p>
                  </div>

                  {/* Clean Branded RestaurantFlow URL */}
                  <div className="flex items-center gap-1.5 bg-slate-900 border border-slate-800 rounded-xl p-1">
                    <span className="text-[11px] font-mono text-brand-400 truncate px-2 flex-1 text-left">
                      github.com/.../RestaurantFlow.apk
                    </span>
                    <button
                      type="button"
                      onClick={handleCopyApkLink}
                      className="px-2 py-1 rounded-lg bg-brand-500 text-slate-950 font-bold text-xs flex items-center gap-1 hover:bg-brand-400 transition flex-shrink-0"
                    >
