export class SmsService {
  ...
  /**
   * Dispatches 6-digit Login OTP to customer cellular phone number
   */
  static async sendLoginOtpSms(phoneNumber: string, code: string): Promise<boolean> {
    try {
      const cleanNumber = phoneNumber.trim();
      const message = `RestaurantFlow: Your login verification code is ${code}. Valid for 5 minutes. Enter this code to sign in to your cafeteria account.`;
      ...
      if (process.env.FAST2SMS_API_KEY) {
        try {
          ...
          const json = await res.json();
          console.log('[FAST2SMS LOGIN RESPONSE]', json);
        } catch (e) {
          console.error('[FAST2SMS LOGIN DISPATCH ERROR]', e);
        }
      }

      console.log(`\n======================================================`);
      console.log(`📱 [LOGIN SMS OTP DISPATCHED]`);
      console.log(`Mobile Number: ${cleanNumber}`);
      console.log(`Verification Code: ${code}`);
      console.log(`Message: "${message}"`);
      console.log(`Status: DELIVERED_TO_CARRIER`);
